    ...
    if selected_names:
        ...
        with tab_pairing:
            ...
            if st.button("🎯 Load Pitch Pairing", type="primary",
                ...
                with st.spinner(f"Loading {pp_pitcher} full season data..."):
                    _all_dates = idx_df["GameDate"].dropna()
                    _pp_min = _all_dates.min()
                    _pp_max = _all_dates.max()
                    try: _pp_min = _pp_min.date()
                    except: pass
                    try: _pp_max = _pp_max.date()
                    except: pass
                    pp_full_df = load_team_data(team_name, _pp_min, _pp_max)
                    if pp_full_df is not None and not pp_full_df.empty:
                        pp_df = get_team_pitches(pp_full_df, team_name, _pp_min, _pp_max)
                        ...
                        pp_clean = pp_df[["PitchType","HorzBreak","InducedVertBreak","RelSpeed","PlateLocSide","PlateLocHeight"]].dropna(subset=["HorzBreak","InducedVertBreak","RelSpeed"])
                        profiles = {}
                        ...
                        if profiles:
                            ...
                            st.success(f"Loaded {pp_pitcher} — {len(profiles)} pitch types, {len(pp_df)} pitches")
                        else:
                            st.warning("Not enough data (need 5+ per pitch type).")
...
p.sub{margin:0 0 10px;font-size:11px;color:#8b949e;text-align:center;}
#ctrls{display:flex;gap:10px;align-items:center;margin-bottom:10px;flex-wrap:wrap;justify-content:center;}
label{font-size:12px;color:#8b949e;}
select{background:#161b22;color:#e6edf3;border:1px solid #30363d;border-radius:6px;padding:3px 7px;font-size:12px;}
...
#tip{position:fixed;background:#1c2128;border:1px solid #30363d;border-radius:6px;
     padding:7px 9px;font-size:11px;pointer-events:none;display:none;z-index:100;line-height:1.7;}
</style></head><body>
<div id="wrap">
  <h3>🎯 Pitch Pairing — """ + pp_pitcher + """</h3>
  <p class="sub">Drag ★ (anchor) anywhere in the zone. All pitches move relative to it based on movement differences.<br>
  Catcher POV: left = inside RHB, right = outside RHB.</p>
  <div id="ctrls">
    <div><label>Anchor: </label><select id="anc"></select></div>
    <div><label>Lines: </label><input type="checkbox" id="lines" checked></div>
    <div><label>Bubbles: </label><select id="bub"><option value="var">By variance</option><option value="fix">Fixed</option></select></div>
  </div>
...
<div id="tip"></div>
<script>
const P=""" + profiles_json + """;
const C=""" + colors_json + """;
const pts=Object.keys(P);
let anc=pts[0], ancPos=null;

const VX1=-2,VX2=2,VY1=0.3,VY2=4.7;
const W=500,H=540,PAD=38;

function tc(fx,fy){
  const cx=PAD+((-fx-VX1)/(VX2-VX1))*(W-2*PAD);
  const cy=H-PAD-((fy-VY1)/(VY2-VY1))*(H-2*PAD);
  return[cx,cy];
}
function fc(cx,cy){
  const fx=-((cx-PAD)/(W-2*PAD)*(VX2-VX1)+VX1);
  const fy=(H-PAD-cy)/(H-2*PAD)*(VY2-VY1)+VY1;
  return[fx,fy];
}

function initAnc(){const p=P[anc];ancPos={x:p.loc_x,y:p.loc_y};}

const sel=document.getElementById('anc');
pts.forEach(pt=>{const o=document.createElement('option');o.value=o.text=pt;sel.appendChild(o);});
sel.addEventListener('change',()=>{anc=sel.value;initAnc();draw();});

const leg=document.getElementById('legend');
pts.forEach(pt=>{
  const d=document.createElement('div');d.className='li';
  d.innerHTML=`<div class="ld" style="background:${C[pt]}"></div><span>${pt} (${P[pt].count})</span>`;
  leg.appendChild(d);
});
...
function pos(pt){
  const a=P[anc],p=P[pt];
  if(pt===anc)return ancPos;
  return{x:ancPos.x-(p.hb-a.hb)/12, y:ancPos.y+(p.ivb-a.ivb)/12};
}
...
function draw(){
  ...
  for(let x=-2;x<=2;x+=0.5){const[cx]=tc(x,0);ctx.beginPath();ctx.moveTo(cx,PAD);ctx.lineTo(cx,H-PAD);ctx.stroke();}
  for(let y=0.5;y<=4.5;y+=0.5){const[,cy]=tc(0,y);ctx.beginPath();ctx.moveTo(PAD,cy);ctx.lineTo(W-PAD,cy);ctx.stroke();}

  // strike zone
  const[zx1,zy1]=tc(-0.83,3.5),[zx2,zy2]=tc(0.83,1.5);
  ctx.strokeStyle='#e6edf3';ctx.lineWidth=2.5;
  ctx.strokeRect(zx1,zy1,zx2-zx1,zy2-zy1);

  // zone thirds
  ctx.strokeStyle='#8b949e';ctx.lineWidth=0.7;ctx.setLineDash([3,3]);
  [2.167,2.833].forEach(y=>{
    const[,cy]=tc(0,y),[cx1]=tc(-0.83,y),[cx2]=tc(0.83,y);
    ctx.beginPath();ctx.moveTo(cx1,cy);ctx.lineTo(cx2,cy);ctx.stroke();
  });
  ctx.setLineDash([]);

  // home plate (tip DOWN = catcher POV)
  ctx.fillStyle='#c9d1d9';ctx.strokeStyle='#8b949e';ctx.lineWidth=1;
  ctx.beginPath();
  [[-0.708,0.5],[0.708,0.5],[0.708,0.32],[0,0.15],[-0.708,0.32]].forEach(([fx,fy],i)=>{
    const[cx,cy]=tc(fx,fy);i===0?ctx.moveTo(cx,cy):ctx.lineTo(cx,cy);
  });
  ctx.closePath();ctx.fill();ctx.stroke();

  // axis labels
  ctx.fillStyle='#8b949e';ctx.font='10px Segoe UI';ctx.textAlign='center';
  ctx.fillText('← Inside RHB  |  Outside RHB →',W/2,H-6);
  ctx.save();ctx.translate(11,H/2);ctx.rotate(-Math.PI/2);ctx.fillText('Height (ft)',0,0);ctx.restore();

  const showL=document.getElementById('lines').checked;
  const bmode=document.getElementById('bub').value;
  const[ax,ay]=tc(pos(anc).x,pos(anc).y);

  // lines
  if(showL){
    pts.forEach(pt=>{
      if(pt===anc)return;
      const p=pos(pt),[px,py]=tc(p.x,p.y);
      ctx.strokeStyle=C[pt]+'55';ctx.lineWidth=1.5;ctx.setLineDash([4,4]);
  ...
  // bubbles
  pts.forEach(pt=>{
    const p=P[pt],ppos=pos(pt),[cx,cy]=tc(ppos.x,ppos.y);
    const isA=pt===anc;
    let r=bmode==='var'?Math.max(14,Math.min(28,Math.sqrt(p.hb_std**2+p.ivb_std**2)*2.5)):19;
    ctx.shadowColor=C[pt]+'99';ctx.shadowBlur=isA?16:8;
    ctx.fillStyle=C[pt];ctx.beginPath();ctx.arc(cx,cy,r,0,Math.PI*2);ctx.fill();
    ctx.shadowBlur=0;
    ctx.strokeStyle=isA?'#fff':C[pt]+'bb';ctx.lineWidth=isA?3:1.5;
    ctx.beginPath();ctx.arc(cx,cy,r,0,Math.PI*2);ctx.stroke();
    ctx.fillStyle='#fff';ctx.textAlign='center';ctx.textBaseline='middle';
    if(isA){ctx.font=`bold ${Math.round(r*0.9)}px Segoe UI`;ctx.fillText('★',cx,cy);}
    else{
      const ab=pt.replace('FourSeam','4S').replace('TwoSeam','2S').replace('Fastball','FB')
               .replace('Sinker','SI').replace('Cutter','CT').replace('Slider','SL')
               .replace('Curveball','CB').replace('ChangeUp','CH').replace('Sweeper','SW').replace('Splitter','SP');
      ctx.font=`bold ${Math.round(r*0.55)}px Segoe UI`;ctx.fillText(ab,cx,cy);
    }
    ctx.textBaseline='alphabetic';
    ctx.fillStyle='#e6edf3';ctx.font='10px Segoe UI';ctx.textAlign='center';
    ctx.fillText(p.velo.toFixed(1),cx,cy+r+11);
  });
}
...
cv.addEventListener('mousemove',e=>{
  ...
  if(drag){const[fx,fy]=fc(mx,my);ancPos={x:fx,y:fy};draw();return;}
  let found=null;
  pts.forEach(pt=>{const p=pos(pt),[cx,cy]=tc(p.x,p.y);if(Math.hypot(mx-cx,my-cy)<28)found=pt;});
  if(found){
    const p=P[found],ppos=pos(found);
    tip.style.display='block';tip.style.left=(e.clientX+12)+'px';tip.style.top=(e.clientY-10)+'px';
    tip.innerHTML=`<b style="color:${C[found]}">${found}</b><br>Velo: ${p.velo.toFixed(1)} mph<br>HB: ${p.hb.toFixed(1)}"&nbsp;&nbsp;IVB: ${p.ivb.toFixed(1)}"<br>Avg loc: (${ppos.x.toFixed(2)}, ${ppos.y.toFixed(2)})<br>n=${p.count} pitches`;
    cv.style.cursor=found===anc?'grab':'default';
  }else{tip.style.display='none';cv.style.cursor='default';}
});
cv.addEventListener('mousedown',e=>{
  const r=cv.getBoundingClientRect(),mx=e.clientX-r.left,my=e.clientY-r.top;
  const[ax,ay]=tc(pos(anc).x,pos(anc).y);
  if(Math.hypot(mx-ax,my-ay)<30){drag=true;cv.style.cursor='grabbing';}
});
cv.addEventListener('mouseup',()=>{drag=false;cv.style.cursor='default';});
cv.addEventListener('mouseleave',()=>{drag=false;tip.style.display='none';});
document.getElementById('lines').addEventListener('change',draw);
document.getElementById('bub').addEventListener('change',draw);
draw();
</script></body></html>"""
